...
def parse_observation_table(obs_table, target, ela_tol=15., sun_tol=20.):
    ...
    mask = (obs_table['OBJECT'] == target) & (obs_table['shutter'] == 0) &            ((obs_table['mode'] == 16) | (obs_table['mode'] == 13)) &            (obs_table['ELA_MIN'] > ela_tol) & (obs_table['SUN_MIN'] > sun_tol)
    ...
    mask = (obs_table['OBJECT'] == 'DARK') & (obs_table['shutter'] == 1) &            (obs_table['xsc'] == xsc) & (obs_table['ysc'] == ysc) &            (obs_table['EXPOSURE'] <= 2*exptime) & (obs_table['EXPOSURE'] >= 0.5*exptime)
