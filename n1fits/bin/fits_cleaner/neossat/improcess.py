...
def clean_sciimage(filename, darkavg, xsc, ysc, xov, yov, snrcut, fmax, xoff, yoff, T, info, bpix, darkmode, **kwargs):
    """"""
    ...
    if dark:
        ...
        if darkmode == 'range':
            newdark, _, _ = scale_image_range(darkavg, scidata_cor, **kwargs)
        elif darkmode == 'zscale':
            newdark, _, _ = scale_image_zscale(darkavg, scidata_cor, **kwargs)
        elif darkmode == 'logspace':
            newdark, _, _ = scale_image_logspace(darkavg, scidata_cor, **kwargs)
        elif darkmode == 'npbin':
            newdark, _, _ = scale_image_npbin(darkavg, scidata_cor, **kwargs)
        else:
            msg = 'Unknown darkmode parameter: {}'.format(darkmode)
            raise ValueError(msg)
